...
@section('content')
    <main >
        ...
        <div class="main_page_title">
            <div class="common_pagetitlebig" style="width: 30%;">{{$pageTitle}}</div>
            <div class="btns_rightimport" style="display: inline-flex">
                <div><strong>Total Disbursed : </strong><span id="totalDisbursedAmount_amount" class="text-danger" style="font-weight: 700;">{{$totalApprovedAmount}}</span></div>
                <div>&nbsp;|&nbsp;</div>
                <div><strong>Total PF Received : </strong><span id="totalProcessingFee_amount" class="text-danger" style="font-weight: 700;">{{$totalApprovedAmount}}</span></div>
                <div>&nbsp;|&nbsp;</div>
                <div><strong>Total Principal Deposit : </strong><span id="totalPrincipalDeposited_amount" class="text-danger" style="font-weight: 700;">{{$totalApprovedAmount}}</span></div>
                <div>&nbsp;|&nbsp;</div>
                <div><strong>Total Interest: </strong><span id="totalInterestGross_amount" class="text-danger" style="font-weight: 700;">{{$totalApprovedAmount}}</span></div>
                <div>&nbsp;|&nbsp;</div>
                <div><strong>Total OutStanding : </strong><span id="totalOutStandingBalance_amount" class="text-danger" style="font-weight: 700;">{{$allOutstandingAmount}}</span></div>
            </div>
        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="block">
                                        <span>Loan Type</span>
                                        <select id="loanreportFilter" class="form-select mt-1.5 w-full rounded-full border border-slate-300 bg-white px-4 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
                                            <option value="none">All Loans</option>
                                            <option value="0">Outstanding Loan</option>
                            ...
                            <div class="col-md-3">
                                <button type="button" onclick="filterAumReports();" class="btn btn-block mt-4 rounded-full bg-danger font-medium text-white hover:bg-danger-focus focus:bg-danger-focus active:bg-danger-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
                                    Search
                                </button>
                            ...
                            <div class="col-md-2 text-right">
                                ...
                                <a id="exportdatahref" href="{{route('adminExportReports',['page'=>'aum-report'])}}/?loanreportFilter=none" class="btn btn-info mt-4 text-white"> Export Data</a> 
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    //     var loanreportFilter = $('#loanreportFilter').val();
    //     $.post('{{route('adminExportReports',['page'=>'aum-report'])}}', {
    //             "_token": "{{ csrf_token() }}", 
    //             loanreportFilter: loanreportFilter 
    //         }, function(data) {
    //             console.log('object');
    //         }
    //     );
    ...
    function resetFilter(){
        ...
        function filterAumReports()
        {
            ...
            url.searchParams.set("loanreportFilter", loanreportFilter);
            ...
            $.post('{{route('filterAumReports')}}',{
                ...
                $("#totalProcessingFee_amount").text($("#totalProcessingFee").text() != '' ? $("#totalProcessingFee").text() : 0 );
                $("#totalPrincipalDeposited_amount").text($("#totalPrincipalDeposited").text() != '' ? $("#totalPrincipalDeposited").text() : 0 );
                $("#totalInterestGross_amount").text($("#totalInterestGross").text() != '' ? $("#totalInterestGross").text() : 0 );
                $("#totalDisbursedAmount_amount").text($("#totalDisbursedAmount").text() != '' ? $("#totalDisbursedAmount").text() : 0 );
                $("#totalOutStandingBalance_amount").text($("#totalOutStandingBalance").text() != '' ? $("#totalOutStandingBalance").text() : 0 );
            });
        }
