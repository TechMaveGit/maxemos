...
Route::get('/test-raw',function(){
    dd('--');
    // $allrawloan = DB::table('apply_loan_histories')->where('loanCategory','3')->where('status','customer-approved')->get();
    if($allrawloan){
        $insertdata = [];
        foreach($allrawloan as $rawl){

            $jsonData = json_decode($rawl->principleChargesDetails,true);

            if($rawl->principleChargesDetails){
            $insertdata[] = ['loanid'=>$rawl->id,'userId'=>$rawl->userId,'plateform_fee'=>$jsonData['plateformFee'],'renewal_from'=>$rawl->validFromDate,'renewal_to'=>$rawl->validToDate,'txn_date'=>$rawl->disbursedDate??$rawl->validFromDate];
            // 
            }
        }
    }
