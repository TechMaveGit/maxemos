@if (auth()->user() && auth()->user()->email == 'admin@gmail.com')
    ...
    <body class="js">
        ...
        <section class="py-4">
                        ...
                        <div class="col-md-12 text-center" style="display: flex;align-items: center;">
                            ...
                            <div style="display: flex;align-items: center; width:80%;">
                                <label for="exampleFormControlInput1"
                                    style="width: 34%;margin-bottom: 0%;font-size: 28px;font-weight: bold;"
                                    class="form-label">Loan #LF0{{ $loanDetails->id }} </label>
                                <select class="contact-one__form-input" id="loanStatusCheck" name="loanData">
                                    @if ($rawLoanDetails)
                                        <option {{ $rawLoanDetails->isAdminApproved == 'approved' ? 'selected' : '' }}
                                            value="approved">Approved</option>
                                        <option {{ $rawLoanDetails->isAdminApproved == 'rejected' ? 'selected' : '' }}
                                            value="rejected">Rejected</option>
                                        <option
                                            {{ $rawLoanDetails->isAdminApproved == 'need update' ? 'selected' : '' }}
                                            value="need update">Need Update</option>
                                    @else
                                        <option {{ $loanDetails->isAdminApproved == 'approved' ? 'selected' : '' }}
                                            value="approved">Approved</option>
                                        <option {{ $loanDetails->isAdminApproved == 'rejected' ? 'selected' : '' }}
                                            value="rejected">Rejected</option>
                                        <option {{ $loanDetails->isAdminApproved == 'need update' ? 'selected' : '' }}
                                            value="need update">Need Update</option>
                                    @endif
                                </select>
                            </div>
            ...
            $pendingDisbmentRequest = DB::table('raw_materials_loan_requests')->where(['loanId'=>$loanDetails->id])->orderBy('created_at','DESC')->get(); 
                    ...
                    <div class="col-lg-12 card card-body my-4" id="btnhistory">
                        <h3 style="font-size: 21px;text-align: left;font-weight: bold;" class="mb-2">Disbursement
                            History</h3>
                            ...
                            <div class="customerprofile_picture">
                                <div class="pgotoof_customer">
                                    ...
                                    <div class="profile_address_content">
                                        ...
                                        <div class="staff-email">Customer Id: {{ $userloggedData->customerCode }}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <ul class="nav nav-pills" id="pills-tab" role="tablist">
                                        <li class="nav-item">
                                            <a class="nav-link active" id="pills-home-tab" data-toggle="pill"
                                                ...
                                                aria-selected="true">Loan Details</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" id="pills-profile-tab" data-toggle="pill"
                                                href="#pills-profile" onclick="userDashboardHtml('application')"
                                        ...
                                        @if (isset($allLoans) && $allLoans && count($allLoans) > 0)
                                            <select class="optgroup_test" id="selectedLoanId"
                                                ...
                                                @foreach ($allLoans as $loan)
                                                    @if ($loanDetails)
                                                        <option {{ $loanDetails->id == $loan->id ? 'selected' : '' }}
                                                            value="{{ $loan->id }}">LF0{{ $loan->id }}
                                                        </option>
                                                    @elseif($previewnamw != $loan->name)
                                                        <optgroup label="{{ $loan->name }}">
                                                    @endif
                                                    <option {{ $selectedLoan == $loan->id ? 'selected' : '' }}
                                                        value="{{ $loan->id }}">LF0{{ $loan->id }}</option>
                                                    @php
                                                        $previewnamw = $loan->name;
                                                    @endphp
                                                    @if ($previewnamw != $loan->name)
                                                        </optgroup>
                                                    @endif
                                                @endforeach
                                            </select>
                                ...
                                @else
                                    <div class="loan_apply mt-4" style="display: block;">
                                        <div class="img_box my-2">
                                            <img src="{{ asset('assets/web') }}/asset/img/apply-loan-123.png"
                                                alt="">
                                        </div>
        ...
        <!-- Modal -->
        <div class="modal fade modal_disbursement" id="exampleModal" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="changePasswordBtn" class=" btn-primary btnpop_frmsubmit">Save
                            Change</button>
                    </div>
                </div>
            </div>
        ...
        <script>
            function userDashboardHtml(requestType) {
                $('#userDashboardRequestedHtml').html(
                    '<center><img src="{{ env('LOADERIMG') }}" style="margin-top:20px;" /></center>');
        ...
        <div style="/* position: absolute; */ bottom: 0px; width: 100%; z-index: 10000; text-align: center;">
            <h6 style="padding: 18px 30px; background-color: aliceblue; margin: 0px;">Feel free to contact us in case
                of any queries. Helpline Number - 7827218200</h6>
        </div>
            ...
            <div class="card" style="width: 40%;text-align: center;">
                <div class="card-body">
                    <img src="{{ asset('assets/web') }}/asset/img/logo/maxemo-logo.png" alt=""
                        style="width: 300px;margin: 26px 0px 15px;">
