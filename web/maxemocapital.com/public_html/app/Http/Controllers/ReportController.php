        ...
        if (count($results)) {
            ...
            foreach ($results as $row) {
                ...
                if($row->loanCategory == 8){
                    $totalEMIAmount += $row->netInterest;
                    $htmlStr .= '<td>' . number_format($row->netInterest,2) . '</td>';
                }else{
                    if(isset($row->interestAmountPayble) && $row->interestAmountPayble){
